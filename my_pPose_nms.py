...
def pose_nms(bboxes, bbox_scores, pose_preds, pose_scores):
    ...
    #print(np.array(ori_bboxes).shape, 'zzx')
    xmax = bboxes[:, 2]
    xmin = bboxes[:, 0]
    ymax = bboxes[:, 3]
    ymin = bboxes[:, 1]
    ...
    for j in range(len(pick)):
        ...
        if (1.5 ** 2 * (xmax - xmin) * (ymax - ymin) < areaThres):
            continue
        #print(int(np.min(ori_bboxes[merge_id, 0].numpy())), merge_id,'zzx1')
        final_result.append({
            'keypoints': merge_pose - 0.3,
            'kp_score': merge_score,
            'proposal_score': torch.mean(merge_score) + bbox_scores_pick[j] + 1.25 * max(merge_score),
            'bbox': [int(np.min(ori_bboxes[merge_id, 0].numpy())),int(np.min(ori_bboxes[merge_id, 1].numpy())),
                     int(np.max(ori_bboxes[merge_id, 2].numpy())),int(np.max(ori_bboxes[merge_id, 3].numpy()))]
        })
